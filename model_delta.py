...
for DATASET in DATASETS:
	with open("data/" + DATASET + ".csv", 'rb') as rawcsv:
		...
		# Since our transformation (delta since last timestep)
		# is essentially a linear transformation of the data, 
		# we can retain our error metric. 
		# i.e. the difference between real and predicted realities
		# is the same as the difference between real and 
		# predicted deltas, since the "prior" in both cases
		# is the same.
		...
		for ii in HMM_CLASSES:
			...
			for i in range(0, ii):
				print(hmm_model.means_[i])

			# calculate year "centers"
			centers = np.zeros((ii,5))
			for i in range(0, ii):
				select = hmm_chain[1::2] == i
				avg_tx = np.mean(dates[1::2,1][select])
				avg_ty = np.mean(dates[1::2,2][select])
