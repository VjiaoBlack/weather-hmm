		...
		for ii in [2,3,4,5,6,10]:
			...
			for j in categories.keys():
				...
				for i in range(0, ii):
					...
					print("class " + str(i) + " centers around " + str(center_date.month) + " " + str(center_date.day) + " std:" + str(center_std) + str(np.round(center_temp)) + " std:" + str(np.round(std_temp)))
					time = time * 12.0 + 1.0

					yvar_mean = center_temp

					plt.plot(time, yvar_mean, marker='.', linewidth=1, c=cmap(float(i)/float(ii-1)))
					plt.scatter(MDWdata_[select][:,0], adata[select][:,j], s=1, marker='.', linewidth=1, c=cmap(float(i)/float(ii-1)), alpha=0.5) 
					plt.annotate(str(i), (time, yvar_mean))
					
				print
				plt.show()
				title = DATASET + "-hmm-dateclass-" + categories[j] + str(ii) + ".png"
